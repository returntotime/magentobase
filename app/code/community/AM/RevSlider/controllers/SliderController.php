...
class AM_RevSlider_SliderController extends Mage_Adminhtml_Controller_Action{
    ...
    public function saveAction(){
        if ($data = $this->getRequest()->getPost()){
            ...
            if (isset($data['slider_id']) && is_numeric($data['slider_id'])){
                ...
                if ($model->checkAlias($data['alias'])){
                    $this->_getSession()->addError($this->__('Slider alias already esixts'));
        ...
        if ($data = $this->getRequest()->getPost()){
            ...
            if ($data['layers']){
                ...
                try{
                    ...
                    Mage::app()->getCache()->clean('matchingTag', array(AM_RevSlider_Model_Slider::CACHE_TAGS));
                    if ($back){
                        $url = $this->getUrl('*/*/addSlide', array(
                            'sid' => $data['slider_id'],
                            'id' => $model->getId()
                        ));
                    }else {
    ...
    protected function _processZipImport($filePath){
        ...
        if (isset($sliderArray['params'])) {
            ...
            if (!empty($sliderArray['params']['alias'])){
                $slider->loadByAlias($sliderArray['params']['alias']);
            }

            $slider->setData(array(
                'id'        => $slider->getId(),
                'title'     => !empty($sliderArray['params']['title']) ? $sliderArray['params']['title'] : 'N/A',
                'alias'     => !empty($sliderArray['params']['alias']) ? $sliderArray['params']['alias'] : $helper->uniqHash(),
                'status'    => 1,
