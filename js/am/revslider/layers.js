...
RevLayers.prototype = {
    ...
    videoParams: 'width|height|fullwidth|cover|loop|control|args|autoplay|autoplayonlyfirsttime|nextslide|mute|force_rewind|disable_on_mobile|preload'.split('|'),
    ...
    preview: function(url){
        if (this.form && this.form.validate()){
            ...
            form.action = url;
            var layers = $('layers') || new Element('input', {type: 'hidden', id: 'layers', name: 'layers'});
            layers.value = JSON.stringify(this.layers);
            Element.insert(form, layers);
            form.target = '_blank';
            form.submit();
        }
    },
    ...
    save: function(back){
        if (this.form && this.form.validate()){
            ...
            new Ajax.Request(url, {
                ...
                parameters: data,
                onSuccess: function(transport){
                    if (back === true && transport.responseText.indexOf('http') === 0) {
